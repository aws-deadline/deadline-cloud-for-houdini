...
def installer_path():
    path = "DeadlineCloudForHoudiniSubmitter-{platform}-installer.{ext}"
...
def _validate_files(installation_path: Path) -> None:
    ...
    assert "_version.py" in submitter_module_dir


# TODO: Add a similar test to above for valiating the HOUDINI_PACKAGE_DIR env variable is set by the installer
...
def test_default_location(installer_path: Path):
    ...
    --prefix <prefix>                           Installation Directory
                                                Default: /home/<user>/DeadlineCloudForHoudiniSubmitter
    """
    # GIVEN
    default_install_location = Path("~/DeadlineCloudForHoudiniSubmitter").expanduser()
